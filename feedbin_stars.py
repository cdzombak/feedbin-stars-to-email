...
def run(feedbin_api, mailgun_api, from_email, to, dry_run):
    ...
    for entry in entries:
        ...
        html_body = '<strong><a href="{:s}">{:s}</a></strong><br><br>'.format(
            url, title
        )
        text_body = url + "\r\n\r\n"
        if len(summary) > 0:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Send your starred Feedbin entries to an email address."
    )
    parser.add_argument(
        "--dry-run",
        type=str2bool,
    ...
    parser.add_argument(
        ...
        default=None,
        dest="from_email",
        help='Email to send from, eg. "User <mailgun@YOUR_DOMAIN_NAME>". Required.',
    )
    parser.add_argument(
        "--to",
        type=str,
        default=None,
        help='Address to send email to, eg. "things@example.com". Required.',
    )
